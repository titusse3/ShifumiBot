...
// var GameStatu = false ;//Etat de la games
...
//     };
//     GameStatu = true;
//     rep_p1 = setTimeout(jour_pas_rep_game, TimeData["anwsert time on Game"], first_player);
//     rep_p2 = setTimeout(jour_pas_rep_game, TimeData["anwsert time on Game"], second_player);
// };
...
//         }
//         else{
//             message_tchat(`@${UserCommande} tu a ${new Date() - user[0].Imune <= TimeData["imunity length"] ? Math.round(((TimeData["imunity length"] - (new Date() - user[0].Imune)) / 60000)%60) + " minute" : "plus"} d'Imuniter`);
//         }
//     } catch (e) {
//         console.error(e);
//     } finally {
...
//         }
//         else {
//             await collection.updateOne({UserId : UserIdPenal.data[0].id}, {$setOnInsert:{User : UserPasRep,UserId : UserIdPenal.data[0].id, Victoire:0, Game : 0, Imune : new Date(), Elo : TimeData["start elo"] + EloArray[1]}}, {upsert : true});
//         }
//     } catch (e) {
//         console.error(e);
//     } finally {
...
//         client.say(target,`/timeout ${first_player} ${TimeData["timeout length"]} T'a pas rep `);
//     }else{
//         Resultat(first_player, second_player);
//         client.say(target,`/timeout ${second_player} ${TimeData["timeout length"]} T'a pas rep `);
//     }
//     restart_game()
// };
...
//         K1 = G[0].Game > TimeData["placement Game number"] ? 20 : 40;
//         K2 = P[0].Game > TimeData["placement Game number"] ? 20 : 40;
...
//         K1 = G[0].Game > TimeData["placement Game number"] ? 20 : 40;
//         Elo.push(K1*(1-Probability(G[0].Elo, TimeData["start elo"])));
//         Elo.push(K2*(0-Probability(TimeData["start elo"], G[0].Elo)));
//     }
//     else if(G.length === 0  && (P.length !== 0 && P[0].Elo !== undefined)){
//         K2 = P[0].Game > TimeData["placement Game number"] ? 20 : 40;
//         Elo.push(K1*(1-Probability(TimeData["start elo"], P[0].Elo)));
//         Elo.push(K2*(0-Probability(P[0].Elo, TimeData["start elo"])));
//     }
//     else {
//         Elo.push(K1*(1-Probability(TimeData["start elo"], TimeData["start elo"])));
//         Elo.push(K2*(0-Probability(TimeData["start elo"], TimeData["start elo"])));
//     };
...
//         if (Joueur1.length === 0){
//             await collection.updateOne({UserId : IdUser1}, {$inc:{Victoire:1, Game : 1}, $setOnInsert : {User : Gagnant, UserId : IdUser1, Elo : TimeData["start elo"]+EloArray[0]}}, {upsert : true});
//         }else{
//             await collection.updateOne({UserId : IdUser1}, {$inc:{Victoire:1, Game : 1, Elo : EloArray[0]}, $set : {User : Gagnant}}, {upsert : true});
//         };
//         if (Joueur2.length === 0){
//             await collection.updateOne({UserId : IdUser2}, {$inc:{Victoire:0, Game : 1}, $set : {Imune : new Date()}, $setOnInsert : {User : Perdant, UserId : IdUser2, Elo : TimeData["start elo"]+EloArray[1]}}, {upsert : true});
//         }else{
...
//         if(User.length === 1 && new Date() - User[0].Imune <= TimeData["imunity length"]){
//             message_tchat(`Partie annulée car ${second_player} n'a pas accepté le match, de plus il a une imuniter il n'auras donc pas de penaliter !`);
//             restart_game();
//             restart_redemption(RedemeptionID, true);
//         }else{
//             await Penaliter(second_player, first_player);
//             message_tchat(`@${second_player} n'a pas accepter le match, il a donc perdu de l'elo !`)
//             restart_game();
//             restart_redemption(RedemeptionID, true);
//         };
//     } catch (e) {
...
//                     message_tchat(`@${first_player} tu ne peux pas défier le bot !`);
//                     restart_game();
//                     restart_redemption(RedemeptionID, true);
//                     return;
//                 case first_player:
//                     message_tchat(`@${first_player} tu ne peux pas faire une partie avec toi même`);
//                     restart_game();
//                     restart_redemption(RedemeptionID, true);
//                     return;
//             };
...
//                     message_tchat(`La partie ne va pas commencer car @${second_player} n'est pas la !`)
//                     restart_game();
//                     restart_redemption(RedemeptionID, true);
//                 }else{
//                     message_tchat(`${second_player} veux tu affronter @${first_player} ? accepte/refuse`);
//                     GameStatu = "waiting response";
//                     temps_de_reponse = setTimeout(J2PasRep, TimeData["anwsert time acceptation"]);//on stocke la fct qui s'active si le j2 ne repond pas dans le temps inpartie
//                 };
//                 return;
//             });
//         };
...
// function commandeHandler(target, context, msg, self){// fonction appeler a chaque message du tchat 
...
//     if (!GameStatu && message[0] === "!"){//regarde utilisation commande
//         switch(message){
//             case "!opgg" :
...
//         };
//     };
//     if (context['display-name'].toLowerCase() === second_player && GameStatu == "waiting response"){//recuperation ou non de l'accaptations ou non de la partie par le P2 
...
//                 clearTimeout(rep_p1);
//                 final_msg = VerifRep(message);
//                 if (final_msg === -1){
//                     message_tchat(`@${second_player} a gagner car @${first_player} n'a pas bien répondu`);
//                     Resultat(second_player, first_player);
//                     client.say(target,`/timeout ${first_player} ${TimeData["timeout length"]} T'a perdu`);
//                     restart_game();
...
//                     Resultat(first_player, second_player);
//                     client.say(target,`/timeout ${second_player} ${TimeData["timeout length"]} T'a perdu`);
//                     restart_game();
//                     restart_redemption(RedemeptionID, true);
//                     return;
//                 };
